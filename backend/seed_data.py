...
# Real Shymkent addresses with true GPS coordinates
shymkent_locations = [
...
# Clear old defects to cleanly reseed
db.query(DefectReport).delete()
...
for idx, (addr, lat, lng, d_type, sev) in enumerate(shymkent_locations, start=1):
    ...
    # Scatter 35 defects realistically over time
    if idx <= 10:
        # Last 24 hours (1 day period)
        days_ago = 0
    elif idx <= 20:
        # Last 7 days
        ...
        # 30-360 days (12 months / All time)
...
db.commit()
print('Successfully seeded', db.query(Defect).count(), 'Shymkent defects with verified street coordinates!')
db.close()
