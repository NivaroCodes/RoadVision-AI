...
export default function MyDefectsPage() {
  ...
  return <div className="mx-auto max-w-5xl space-y-6">
    ...
    <div className="grid gap-4 sm:grid-cols-2">{query.data?.map((defect) => <article key={defect.id} className="rounded-xl border bg-card p-5 shadow-sm"><div className="flex items-start justify-between gap-4"><div><p className="text-xs text-muted-foreground">Обращение</p><h2 className="text-xl font-bold">#{defect.id}</h2></div><span className="rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold">{labels[defect.status]}</span></div><div className="mt-5 space-y-3 text-sm text-muted-foreground"><p className="flex gap-2"><MapPin className="h-4 w-4 shrink-0" />{defect.address || `${defect.latitude.toFixed(5)}, ${defect.longitude.toFixed(5)}`}</p>{defect.created_at && <p className="flex gap-2"><Clock3 className="h-4 w-4" />{new Date(defect.created_at).toLocaleString('ru-RU')}</p>}<p>Приоритет: {defect.priority} · подтверждений: {defect.confirmation_count}</p></div></article>)}</div>
  </div>;
}
