...
interface DefectsRegistryProps {
  isLoading?: boolean
}

export function DefectsRegistry({ isLoading = false }: DefectsRegistryProps) {
  ...
  const handleSave = (updatedDefect: DefectMarker) => {
    setDefects((current) => current.map((defect) => defect.id === updatedDefect.id ? updatedDefect : defect))
  ...
  return (
    <section className="space-y-4" aria-labelledby="defects-registry-title">
      ...
      {isLoading ? <DefectsTableSkeleton /> : filteredDefects.length > 0 ? <DefectsTable defects={filteredDefects} onSelect={setSelectedDefect} /> : <EmptyState icon={SearchX} />}
