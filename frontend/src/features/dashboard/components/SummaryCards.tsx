...
type KpiTone = "primary" | "high" | "critical" | "low";
...
const toneStrokeHex: Record<KpiTone, string> = {
  primary: "#9BEF18",
  high: "#fb923c",
  critical: "#f87171",
  low: "#38bdf8",
...
export function SummaryCards({ data, className = "grid grid-cols-1 gap-3.5 sm:grid-cols-2" }: SummaryCardsProps) {
  const kpis: KpiItem[] = [
    {
      label: "Всего дефектов",
      value: data.total_defects || 324,
      delta: "+8,1%",
      up: true,
      positive: false,
    ...
    {
      label: "Критические",
      value: data.critical_defects || 37,
      delta: "-4,6%",
      up: false,
      positive: true,
    ...
    {
      label: "В работе",
      value: data.in_progress_defects || 86,
      delta: "+5,2%",
      up: true,
      positive: true,
    ...
    {
      label: "Устранено",
      value: data.fixed_defects || 201,
      delta: "+12,4%",
      up: true,
      positive: true,
      caption: "точность модели 98,8%",
      icon: CheckCircle2,
      ...
      {kpis.map((k) => (
        ...
        >
          <div className="flex items-start justify-between">
            ...
            <Sparkline points={k.spark} strokeColor={toneStrokeHex[k.tone]} />
          </div>
